...
class TestRegressionTreeTrainer extends FunSuite {
  ...
  test("train/test") {
    ...
    val columns = train.toSortedColumns[Double,Observation with Feature with Label[Double]]
  ...
  test("2-d linear") {
    var rows = Vector(ObservationRowLabel(0, Array(0, 0), 0.))
    rows = rows :+ ObservationRowLabel(1, Array(0, 1), 1.)
    rows = rows :+ ObservationRowLabel(2, Array(1, 0), 2.)
    rows = rows :+ ObservationRowLabel(3, Array(1, 1), 3.)
  ...
  def testTrainer(config: DecisionTreeTrainConfig, rows: IndexedSeq[ObservationRowLabel[Double]]) = {
    val columns = rows.toSortedColumns[Double,Observation with Feature with Label[Double]]
