...
class TestStochasticGradientBoostTrainer extends FunSuite {
  test("SGB - toy 1d") {
    ...
    val trainer = new StochasticGradientBoostTrainer[Boolean](config, cost, rows.map(r => ObservationLabel(r.rowId, r.label)), rows.toSortedColumns[Boolean, ObservationLabelFeature[Boolean]](_.singleFeature(_)))
    val tol = 1.0e-8
  ...
  test("SGB - toy 2d") {
    ...
    val cost = new LogLogisticLoss().asInstanceOf[CostFunction[Boolean, Observation with Label[Boolean]]]
    val trainer = new StochasticGradientBoostTrainer(config, cost, rows.map(r => ObservationLabel(r.rowId, r.label)), rows.toSortedColumns[Boolean, ObservationLabelFeatureScore[Boolean]](_.singleFeature(_)))
  ...
  test("SGB - 2d") {
    ...
    val cost = new LogLogisticLoss().asInstanceOf[CostFunction[Boolean, Observation with Label[Boolean]]]
    val trainer = new StochasticGradientBoostTrainer(config, cost, rows.map(r => ObservationLabel(rowId = r.rowId, label = r.label)), rows.toSortedColumns[Boolean, ObservationLabelFeatureScore[Boolean]](_.singleFeature(_)))
    val models = (0 until config.iterationCount) map (i => { trainer.nextIteration(); trainer.model })
    val errorCount = models map (m => rows.count(r => m.eval(r.features) > 0 ^ r.label))
    val losses = for (m <- models) yield {
      for (row <- rows) { row.score = m.eval(row.features) }
