...
class TestDecisionTreeTrainNode extends FunSuite with BeforeAndAfter {
  ...
  def init = {
    ...
    rows = (0 until size).map(i => new LabeledFeatureRow[Double](i, Vector(features(i)), 1.0, features(i)))
  ...
  test("RegressionTreeTrainer") {
    ...
    val loss1 = rows.map(r => math.pow(r.label - model1.eval(r), 2)).sum

    val split = trainer.nextIteration()
    val model2 = trainer.model
    val loss2 = rows.map(r => math.pow(r.label - model2.eval(r), 2)).sum
